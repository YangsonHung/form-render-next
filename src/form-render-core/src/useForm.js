...
const useForm = (props) => {
  ...
  const logOnMount = _logOnMount || (window.FR_LOGGER && window.FR_LOGGER.logOnMount);
  const logOnSubmit = _logOnSubmit || (window.FR_LOGGER && window.FR_LOGGER.logOnSubmit);
    ...
    try {
      Object.keys(settings).forEach((path) => {
        if (!_flatten.current[path]) {
          ...
          const _newSchema =
            typeof newSchema === 'function' ? newSchema(newFlatten[path].schema) : newSchema;
